package com.example.fist_class
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        texto.text = database[0]
        Toast.makeText(this, "piensa positivo", Toast.LENGTH_LONG).show()
        Log.i("edu.edu.iopienso", "IoPP Loading")
    }

    fun getRandomMessage(): String{
        val size = database.size-1
        var rnd = Random.nextInt(0,size)
        position = rnd
        return database[rnd]
    }

    fun frwdlist(view: View){
        position++
        position = if (position >= database.size) 0 else position
        texto.text = database[position]
    }

    fun backlist(view: View){
        position--
        position = if (position < 0) database.size-1 else position
        texto.text = database[position]
    }
